...
class NaiveBayes:
    ...
    def top_n(self, label, n):
        ...
        """
        labeledDict = self.class_word_counts[label]
        topTokensDict = dict(sorted(labeledDict.items(), key=operator.itemgetter(1), reverse=True))
        topN = list(topTokensDict.items())
        return topN[:n]
    ...
    def log_likelihood(self, bow, label, alpha):
        ...
        sumLog = 0.0
        for word in bow:
            sumLog += math.log(self.p_word_given_label_and_alpha(word, label, alpha))
        return sumLog
    ...
    def log_prior(self, label):
        ...
        """
        return (self.class_total_doc_counts[label])/(sum(self.class_total_doc_counts.values()))
